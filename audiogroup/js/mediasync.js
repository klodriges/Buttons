...
var mediascape = function(_MS_) {
  ...
  function mediaSync(elem, motion, options) {
    ...
    // Need to do trickery on Netflix, make a seek function
    var seekTo = function(target_s) {
      if (window.netflix || _netflix) {
        if (!_netflix) {
          let session = netflix.appContext.state.playerApp.getAPI().videoPlayer.getAllPlayerSessionIds()[0];          
          _netflix = netflix.appContext.state.playerApp.getAPI().videoPlayer.getVideoPlayerBySessionId(session);
          window._netflix = _netflix;
        }
        console.log("Netflix seeking to", target_s * 1000);
        _netflix.seek(target_s * 1000);
      } else {
        elem.currentTime = target_s;
      }
    };
